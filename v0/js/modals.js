...
function openToolForEdit(def){
  ...
  ebDraft = JSON.parse(JSON.stringify({
    ...
    moveable: !def.fixed, w: def.w, h: def.h,
...
function loadDraftIntoForm(){
  ...
  document.getElementById("ebMoveable").value = String(ebDraft.moveable);
  document.getElementById("ebW").value = ebDraft.w;
  document.getElementById("ebH").value = ebDraft.h;
...
function readFormIntoDraft(){
  ...
  ebDraft.moveable = document.getElementById("ebMoveable").value === "true";
  ebDraft.w = parseFloat(document.getElementById("ebW").value);
  ebDraft.h = parseFloat(document.getElementById("ebH").value);
...
function saveCustomElement(){
  ...
  if(state.editingId){
    ...
    const existing = state.customElements[idx];
    ...
    // Stay in edit mode in case the user wants to keep tweaking.
    document.getElementById("ebSubtitle").textContent = `Editing: ${d.label} (saved)`;
  } else {
    // Create new
    ...
    state.zones[id] = {
      x: 50 - d.w/2, y: 50 - d.h/2,
      w: d.w, h: d.h, rotation: 0,
      included: true, activeUse: false,
    };
    evaluate(); render();
    renderCustomElementList();
    // Reset draft for rapid-fire creation but keep the user's tab.
    ebDraft = blankDraft();
    loadDraftIntoForm();
    document.getElementById("ebLabel").focus();
  }
}
...
function saveStructuralElement(){
  ...
  if(state.editingId){
    ...
    ebStructDraft = blankStructuralDraft();
    loadStructuralDraftIntoForm();
    document.getElementById("sbLabel").focus();
  }
}
...
function loadAmenityDraftIntoForm(){
  ...
  document.getElementById("amSize").value = ebAmenityDraft.size;
  document.getElementById("amCoverage").value = ebAmenityDraft.coverage;
  document.getElementById("amMoveable").value = String(ebAmenityDraft.moveable);
}
...
function saveAmenityElement(){
  ...
  if(state.editingId){
    ...
    evaluate(); render();
    document.getElementById("ebSubtitle").textContent = "Editing: " + label + " (saved)";
  } else {
    ...
    ebAmenityDraft = blankAmenityDraft();
    loadAmenityDraftIntoForm();
    document.getElementById("amLabel").focus();
  }
}
...
function wireElementBuilder(){
  ...
  document.getElementById("amSize").addEventListener("input", e=>{
    ebAmenityDraft.size = parseFloat(e.target.value) || 3;
  });
  document.getElementById("amCoverage").addEventListener("input", e=>{
    ebAmenityDraft.coverage = parseFloat(e.target.value) || 0;
  });
  document.getElementById("amMoveable").addEventListener("change", e=>{
    ebAmenityDraft.moveable = e.target.value === "true";
  });
