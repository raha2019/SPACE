...
function evaluate(){
  const flags = [];
  const Z = state.zones;
  const defs = Object.fromEntries(ZONE_DEFS.map(d=>[d.id,d]));
  ...
  // ---------- 1. Object overlap
  const movable = ZONE_DEFS.filter(d=>!d.fixed && d.cat!=="corridor" && d.cat!=="open" && isIncluded(d.id));
  ...
  const highRisk = ZONE_DEFS.filter(d=>d.risk>=4 && isIncluded(d.id));
...
function computeMetrics(){
  ...
  for(const d of ZONE_DEFS.filter(d=>d.risk>=3 && (Z[d.id] && Z[d.id].included !== false))){
    const z = rect(Z[d.id]);
  ...
  for(const d of ZONE_DEFS.filter(d=>d.risk>=4 && (Z[d.id] && Z[d.id].included !== false))){
    const z = Z[d.id];
