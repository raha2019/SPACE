...
function openStructuralForEdit(def){
  // Elements drawn in the Floor Plan Builder reopen that editor (it reloads
  // the whole plan, so every piece is editable together).
  if(def && def.rawDraw && def.rawDraw.source === "fpb" && typeof openFpbModal === "function"){
    openFpbModal();
    return;
  }
  // Elements drawn in the classic wall editor are polygons — edit them there.
  ...
  document.querySelectorAll("#screenChooser .chooser-card").forEach(card=>{
    card.addEventListener("click", ()=>{
      ...
      if(choice === "new-tool") startNewTool();
        ...
        // Walls / floor / doors are drawn in the embedded Floor Plan Builder;
        // fall back to the classic wall editor if the bridge isn't loaded.
        if(typeof openFpbModal === "function") openFpbModal();
        else if(typeof openWallDraw === "function") openWallDraw();
